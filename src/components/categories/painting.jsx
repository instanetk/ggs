...
const Painting = () => {
  ...
  useEffect(() => {
    ...
    async function fetchServices() {
      const { data } = await getServices('painting');
      setService(data);
    }
    fetchServices();
  }, []);
  ...
  return (
    <main>
      <div className="-mt-6 sm:mt-0">
        <SecondaryHero text1={t('painting.1')} color="text-red-500" image="catPainting" />
      </div>
      <div>
        ...
        <div className="px-4 sm:-mt-8 sm:px-20">
          ...
          <div className="mt-6 pb-12 text-gray-800 tracking-wide text-justify text-lg select-none">
            <p className="mt-2 flex-wrap">{t('painting.text')}</p>
          </div>
        </div>
      </div>
