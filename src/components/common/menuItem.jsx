...
const MenuItem = ({ label }) => {
  const icon = (label) => {
    ...
    switch (label) {
      ...
      case 'Chat':
        ...
        className="h-7 w-7 transform scale-125 sm:block flex-shrink-0"
        ...
        />
      </svg><div id="online-sm" className="hidden sm:block relative h-2 w-2 -ml-0 -mt-5">
      <span className="flex h-1 w-1">
        <span className="animate-ping absolute inline-flex h-1 w-1 rounded-full bg-green-400 opacity-75"></span>
        <span className="relative inline-flex rounded-full h-1 w-1 bg-green-500"></span>
      </span>
  ...
  return (
    <React.Fragment>
      <svg
        className="h-6 w-6 sm:block flex-shrink-0"
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor">
